...
import type { Account, DonVi, QuanSoBienCheResult } from "../../types/account";
...
export default function Settings() {
  ...
  const [quanSoBienChe, setQuanSoBienChe] =
    useState<QuanSoBienCheResult | null>(null);
    ...
    const fetchData = async () => {
      try {
        ...
        if (acc.donVi?.maDonVi) {
          const [allUnits, qsbc] = await Promise.all([
            donviService.getDonVi(),
            donviService.getQuanSoBienChe(acc.donVi.maDonVi).catch(() => null),
          ]);
          const unit = allUnits.find((u) => u.maDonVi === acc.donVi!.maDonVi);
          if (unit) setDonVi(unit);
          setChildUnits(getDirectChildUnits(allUnits, acc.donVi.maDonVi));
          setQuanSoBienChe(qsbc);
        }
      } catch (err) {
